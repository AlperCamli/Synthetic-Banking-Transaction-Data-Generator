def generate_transactions(customers_df: pd.DataFrame,
    ...
    Quarterly-adaptive generator:
      ...
      - Segment-specific price ranges; varied purchase tickets (if sample_purchase_amount is defined)
    """
    ...
    for _, cust in customers_df.iterrows():
        ...
        while month_cursor <= end:
            ...
            if start <= s_day <= end and monthly_salary > 0:
                _add_sched(scheduled, s_day, {
                    ...
                    "merchant_id": None, "channel": "branch",
                    "is_recurring": True
                })
            ...
            if monthly_passive > 0:
                ...
                if start <= p_day <= end:
                    _add_sched(scheduled, p_day, {
                        ...
                        "merchant_id": None, "channel": "online",
                        "is_recurring": True
                    })
        ...
        try:
            ...
            # fallback: midpoint of shopping range scaled by income
        ...
        for qs, qe_excl in quarter_windows(start, end):
            ...
            if q_prev_crashed:
                ...
                weekly_rate_q *= 0.90  # fewer random attempts after a crash
            else:
            ...
            try:
                ...
                # use original row weights
                cat_w_q = {}
                for suffix, cat in WEIGHT_COL_TO_CATEGORY.items():
                    if cat == "atm":  # purchases only
                        continue
                    col = f"w_cat_{suffix}"
                    if col in cust:
            ...
            while d < qe_excl:
                ...
                # counts & gentle amount jitter (supports both daily_thin_factors and daily_thin_factor)
                try:
                ...
                var_spend_today = 0.0
                slack = 0.15  # you can raise to 0.30 for larger single tickets
                ...
                for _ in range(k):
                    ...
                    if channel_name == "atm":
                        ...
                        # category (quarter weights) and merchant
                        try:
                            category = pick_category_with_override(cat_w_q)
                        except Exception:
                            # fallback to per-row weights
                            cats, ws = [], []
                            for suffix, cat in WEIGHT_COL_TO_CATEGORY.items():
                                if cat == "atm":
                                    continue
                        ...
                        # amount (rich sampler if available; else fallback)
                        try:
                            amt = sample_purchase_amount(category, seg_ranges, disc_q, m_idx, d)
                        except Exception:
                            # simple fallback: sample within seg range with beta skew
